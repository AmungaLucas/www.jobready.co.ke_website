...
// ─── Page Config ───────────────────────────────────────
const OPP_TYPE = "INTERNSHIP";
const PAGE_TITLE = "Internships & Opportunities in Kenya";
const BREADCRUMB_NAME = "Internships";
const SLUG = "internships";
const META_DESCRIPTION = "Find the latest internships and career opportunities in Kenya. Apply now on JobReady Kenya.";
...
export async function generateMetadata({ searchParams }) {
  ...
  const description = q
    ? `Search results for "${q}" in internships. ${META_DESCRIPTION}`
    : META_DESCRIPTION;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {total.toLocaleString()} {total === 1 ? "internship" : "internships"} available
              {q && (
                <span>
          ...
          <div className="flex-1 min-w-0">
            {opportunities.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-xl shadow-sm">
                ...
                <h2 className="text-lg font-semibold text-gray-700 mb-1">No internships found</h2>
                <p className="text-sm text-gray-500 max-w-md mx-auto">
                  {q
                    ? `We couldn't find any internships matching "${q}". Try different keywords or clear your search.`
                    : "No internships are currently listed. Check back soon!"}
                </p>
                <Link
                  href="/opportunities"
