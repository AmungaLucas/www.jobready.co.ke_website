...
export async function generateMetadata({ searchParams }) {
  ...
  const title = q ? `${q} — Scholarships in Kenya` : PAGE_TITLE;
  const description = q
    ? `Search results for "${q}" in scholarships. ${META_DESCRIPTION}`
    : META_DESCRIPTION;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {total.toLocaleString()} {total === 1 ? "scholarship" : "scholarships"} available
              {q && (
                <span>
          ...
          <div className="flex-1 min-w-0">
            {opportunities.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-xl shadow-sm">
                ...
                <h2 className="text-lg font-semibold text-gray-700 mb-1">No scholarships found</h2>
                <p className="text-sm text-gray-500 max-w-md mx-auto">
                  {q
                    ? `We couldn't find any scholarships matching "${q}". Try different keywords or clear your search.`
                    : "No scholarships are currently listed. Check back soon!"}
                </p>
                <Link
                  href="/opportunities"
