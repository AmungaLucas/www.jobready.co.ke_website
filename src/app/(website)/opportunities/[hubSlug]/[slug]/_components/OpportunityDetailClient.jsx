...
export default function OpportunityDetailClient({ hubSlug: initialHubSlug, slug: initialSlug }) {
  const params = useParams();
  const hubSlug = initialHubSlug || params?.hubSlug;
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        console.error("[OpportunityDetailClient] Fetch error:", err);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50">
        {/* Skeleton hero */}
        <div className="bg-gradient-to-br from-purple-600 via-indigo-600 to-indigo-700 text-white animate-pulse">
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#1a56db] hover:bg-[#1648b8] text-white font-semibold text-sm rounded-xl transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  const similarOpportunities = data.similarOpportunities || [];

  const typeColor =
    typeBadgeColors[opp.opportunityType?.toLowerCase()] || "bg-gray-100 text-gray-700";

  // Derived from company relation
