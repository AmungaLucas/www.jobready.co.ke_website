...
// ─── Page Config ───────────────────────────────────────
const OPP_TYPE = "GRANT";
const PAGE_TITLE = "Grants in Kenya";
const BREADCRUMB_NAME = "Grants";
const SLUG = "grants";
const META_DESCRIPTION = "Find the latest grants in Kenya for NGOs, startups, researchers and community projects. Apply now on JobReady Kenya.";
...
export async function generateMetadata({ searchParams }) {
  ...
  const title = q ? `${q} — Grants in Kenya` : PAGE_TITLE;
  const description = q
    ? `Search results for "${q}" in grants. ${META_DESCRIPTION}`
    : META_DESCRIPTION;
...
// ─── Page Component ─────────────────────────────────────
export default async function GrantsPage({ searchParams }) {
  const sp = await searchParams;
  const q = ((sp.q || "") || "").trim() || "";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {total.toLocaleString()} {total === 1 ? "grant" : "grants"} available
              {q && (
                <span>
          ...
          <div className="flex-1 min-w-0">
            {opportunities.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-xl shadow-sm">
                ...
                <h2 className="text-lg font-semibold text-gray-700 mb-1">No grants found</h2>
                <p className="text-sm text-gray-500 max-w-md mx-auto">
                  {q
                    ? `We couldn't find any grants matching "${q}". Try different keywords or clear your search.`
                    : "No grants are currently listed. Check back soon!"}
                </p>
                <Link
                  href="/opportunities"
