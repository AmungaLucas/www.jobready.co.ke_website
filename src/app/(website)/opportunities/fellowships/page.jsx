...
// ─── Page Config ───────────────────────────────────────
const OPP_TYPE = "FELLOWSHIP";
const PAGE_TITLE = "Fellowships in Kenya";
const BREADCRUMB_NAME = "Fellowships";
const SLUG = "fellowships";
const META_DESCRIPTION = "Find the latest fellowships in Kenya for professionals, researchers and graduate students. Apply now on JobReady Kenya.";
...
export async function generateMetadata({ searchParams }) {
  ...
  const title = q ? `${q} — Fellowships in Kenya` : PAGE_TITLE;
  const description = q
    ? `Search results for "${q}" in fellowships. ${META_DESCRIPTION}`
    : META_DESCRIPTION;
...
// ─── Page Component ─────────────────────────────────────
export default async function FellowshipsPage({ searchParams }) {
  const sp = await searchParams;
  const q = sp.get("q")?.trim() || "";
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: BREADCRUMB_NAME, href: `/opportunities/${SLUG}` },
  ]);
  ...
  function buildQS(overrides = {}) {
    ...
    return qs ? `/opportunities/${SLUG}?${qs}` : `/opportunities/${SLUG}`;
  }
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8 md:py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex flex-wrap items-center gap-1">
          ...
          <span className="text-gray-300 mx-1">/</span>
          <span className="text-gray-700 font-medium">{BREADCRUMB_NAME}</span>
        </nav>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {total.toLocaleString()} {total === 1 ? "fellowship" : "fellowships"} available
              {q && (
                <span>
          ...
          {/* Search */}
          <form action={`/opportunities/${SLUG}`} method="get" className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <div className="flex-1 min-w-0">
            {opportunities.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-xl shadow-sm">
                ...
                <h2 className="text-lg font-semibold text-gray-700 mb-1">No fellowships found</h2>
                <p className="text-sm text-gray-500 max-w-md mx-auto">
                  {q
                    ? `We couldn't find any fellowships matching "${q}". Try different keywords or clear your search.`
                    : "No fellowships are currently listed. Check back soon!"}
                </p>
                <Link
                  href="/opportunities"
