...
// ─── Page Config ───────────────────────────────────────
const OPP_TYPE = "BURSARY";
const PAGE_TITLE = "Bursaries in Kenya";
const BREADCRUMB_NAME = "Bursaries";
const SLUG = "bursaries";
const META_DESCRIPTION = "Find the latest bursaries in Kenya for secondary and university students. Apply now on JobReady Kenya.";
...
export async function generateMetadata({ searchParams }) {
  ...
  const title = q ? `${q} — Bursaries in Kenya` : PAGE_TITLE;
  const description = q
    ? `Search results for "${q}" in bursaries. ${META_DESCRIPTION}`
    : META_DESCRIPTION;
...
// ─── Page Component ─────────────────────────────────────
export default async function BursariesPage({ searchParams }) {
  const sp = await searchParams;
  const q = sp.get("q")?.trim() || "";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {total.toLocaleString()} {total === 1 ? "bursary" : "bursaries"} available
              {q && (
                <span>
          ...
          <div className="flex-1 min-w-0">
            {opportunities.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-xl shadow-sm">
                ...
                <h2 className="text-lg font-semibold text-gray-700 mb-1">No bursaries found</h2>
                <p className="text-sm text-gray-500 max-w-md mx-auto">
                  {q
                    ? `We couldn't find any bursaries matching "${q}". Try different keywords or clear your search.`
                    : "No bursaries are currently listed. Check back soon!"}
                </p>
                <Link
                  href="/opportunities"
