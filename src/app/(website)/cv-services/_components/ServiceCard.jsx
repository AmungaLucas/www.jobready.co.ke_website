...
export default function ServiceCard({ service }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow">
      ...
      {/* CTA */}
      <a
        href={getWhatsAppLink(service.name, popularTier?.name)}
        target="_blank"
        rel="noopener noreferrer"
        className="block w-full text-center px-5 py-3 rounded-lg bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 transition-colors no-underline"
      >
        Order {service.name}
      </a>
    </div>
