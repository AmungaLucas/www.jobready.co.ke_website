...
function buildWhere(config, q) {
  ...
  // Simple field filter for employmentType, experienceLevel, isRemote
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              {total} {pluralize(vacancyLabel, total)} available
            </p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="absolute inset-0 opacity-0 pointer-events-none">
              {SORT_OPTIONS.map((opt) => (
                <a key={opt.value} href={buildUrl(config.pagePath, params, { sort: opt.value, page: undefined })} aria-label={`Sort by ${opt.label}`} />
              ))}
            </div>
          </div>
        </div>
          ...
          <form action={config.pagePath} method="GET" className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                name="q"
                defaultValue={q}
                placeholder={config.searchPlaceholder || `Search ${config.breadcrumbName.toLowerCase()} jobs by title or keyword...`}
          ...
          <div className="lg:col-span-3">
            {jobs.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <h2 className="text-lg font-bold text-gray-900 mb-2">{config.emptyTitle || `No ${config.breadcrumbName.toLowerCase()} jobs found`}</h2>
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-teal-600 to-teal-700 rounded-xl p-5 text-white shadow-md">
              <div className="text-center">
                <span className="text-3xl mb-2 block">{config.sidebarEmoji || "💼"}</span>
                <h3 className="font-bold text-lg mb-1">{config.sidebarTitle || "Job Alerts"}</h3>
                <p className="text-teal-100 text-sm mb-4 leading-relaxed">
                  {config.sidebarDescription || "Never miss an opportunity. Get a professional CV and be ready when the next one opens."}
                </p>
                <a
